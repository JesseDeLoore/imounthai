...
def test_recipe_price(basic_order: OrderRecipe):
    ...
    assert pytest.approx(float(basic_order.total_price), 0.00001) == price
    assert pytest.approx(float(basic_order.total_vat), 0.00001) == vat
    assert pytest.approx(float(basic_order.total_price), 0.00001) == vat
